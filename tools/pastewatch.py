...
PSBDMP_SEARCH = "https://psbdmp.ws/api/search/{query}"
...
class PasteWatch:
    ...
    def run(target, timeout=10):
        ...
        try:
            resp = requests.get(f"https://pastebin.com/search?q={quote(query)}", timeout=timeout,
        ...
        try:
            resp = requests.get(f"https://psbdmp.ws/api/search/{quote(query)}", timeout=timeout,
                headers={"User-Agent": "Mozilla/5.0 (X11; Linux x86_64; rv:120.0) Gecko/20100101 Firefox/120.0"})
            if resp.status_code == 200:
                data = resp.json()
                if isinstance(data, dict) and data.get("count", 0) > 0:
                    count = data["count"]
                    warning(f"  Found {count} paste(s) via PSBDMP!")
                ...
                info(f"  PSBDMP returned HTTP {resp.status_code}")
        except Exception as e:
            info(f"  PSBDMP search failed: {str(e)[:50]}")
        ...
        try:
            resp = requests.get("https://pastebin.com/archive", timeout=timeout,
                headers={"User-Agent": "Mozilla/5.0 (X11; Linux x86_64; rv:120.0) Gecko/20100101 Firefox/120.0"})
            if resp.status_code == 200:
